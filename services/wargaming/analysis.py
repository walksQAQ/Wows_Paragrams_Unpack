...
def _first(v):
    """取数组/单元素列表的首个标量；标量原样返回（兼容多档位数组字段如 dropTargetAtDistance）。"""
...
PROJECTILE_EXT_MAP = {
    ...
    "Torpedo": {
        ...
        "cols": ("bullet_diameter, alpha_damage, damage, flood_generation, "
                 ...
                 "is_deep_water, deep_water_ignore_classes, distance_of_damage_json"),
        "phs": 16,
        "fields": (
            ...
            lambda r: _json_dumps(r.get("distanceOfDamage"))
        ),
    },
    "DepthCharge": {
...
# ═════════════════════════════════════════════════════════════════════
# WargamingAnalysisStore —— 数据写入器
# ═════════════════════════════════════════════════════════════════════

class WargamingAnalysisStore:
    """将解析后的数据写入结构化数据库表"""
    ...
    def store_ship(self, ship_id: str, raw_data: dict, version_code: str = ""):
        ...
        for mod_key, module_data in raw_data.items():
            ...
            # 空袭（WG：A_AirSupport 模块直接携带字段，扁平结构；兼容含 Armament 子字典的版本）
            if current_cat == "AirSupport":
                for lt in target_letters:
                    combined_stats.setdefault(lt, {}).setdefault("air_support", [])
                src_list = [module_data]
                for k, v in module_data.items():
                    if isinstance(v, dict) and "Armament" in k:
                        src_list.append(v)
                for src in src_list:
                    entry = {
                        "ammo_list": src.get("ammoList", []),
                        "ammo_switch_coeff": src.get("ammoSwitchCoeff", 0),
                        "charges": src.get("chargesNum", 0),
                        "reload_time": src.get("reloadTime", 0),
                        "max_range": _v(src.get("maxDist")) if src.get("maxDist") is not None else None,
                        "min_range": _v(src.get("minDist")) if src.get("minDist") is not None else None,
                        "fly_away_time": _v(src.get("flyAwayTime")),
                        "time_from_heaven": _v(src.get("timeFromHeaven")),
                        "auto_use": _bn(src.get("autoUsage")),
                        "available_buoyancy_states": src.get("availableBuoyancyStates", []),
                    }
                    for lt in target_letters:
                        combined_stats[lt]["air_support"].append(entry)
                continue
            ...
            if current_cat in ("Artillery", "SecondaryArtillery", "ATBA", "AirDefense", "Torpedoes", "DepthChargeGuns"):
                ...
                if current_cat in ("Artillery", "SecondaryArtillery", "ATBA"):
                    ...
                    if current_cat == "ATBA":
                        ...
                        cg = module_data.get("controlGroups") or {}
                        mm = module_data.get("manualModeModifiers") or {}
                        if cg or mm:
                            for lt in target_letters:
                                combined_stats[lt]["_atba_config"] = {
                                    "control_groups": cg,
                                    "manual_mode_modifiers": mm,
                                }
                ...
                if current_cat == "Artillery":
                    # 弹夹炮识别：本文件内联（WG 版，TODO(手动) 逐点调整）
                    burst = _recognize_burst_wg(module_data)
                    if burst:
                        for lt in target_letters:
                            drum_configs[lt] = burst
    ...
    def _write_turret_arcs(self, ship_id: str, raw_data: dict, version_code: str = ""):
        ...
        炮位安装朝向（mount_yaw/mount_pos）**不在此处提取**：3D 查看器绘制射界时
        直接经 ship_models.model_folder 从 assets_data.db 的 skeleton_mounts
        读取挂点解码坐标/四元数，此处两列统一留空。
        """
        rows = []
    ...
    def _write_artillery(self, ship_id: str, letter: str, cs: dict, di: dict | None = None, version_code: str = ""):
        ...
        for key, group in self._weapon_groups(cs.get("artillery", []), "gun_name").items():
            ...
            if di:
                # WG 表 ship_module_artillery_ext 用 switchablemode_* 列（无 Lesta 的
                # is_switchable/is_chargeable/charge_time_* 列），且含 switchablemode_secondary_ammo_list
                conn.execute("INSERT OR REPLACE INTO ship_module_artillery_ext (version_code, ship_id, config_group, module_key, special_mode_name, switchablemode_shots_count, switchablemode_shot_delay, switchablemode_full_reload_time, switchablemode_modifiers_json, switchablemode_secondary_ammo_list) VALUES (?,?,?,?,?,?,?,?,?,?)",
                             (version_code, ship_id, letter, gn, di.get("name", ""),
                              _v(di.get("shots_count"), 0),
                              _v(di.get("shot_delay"), 0),
                              _v(di.get("full_reload_time"), 0),
                              _json_dumps(di.get("modifiers", {})),
                              _json_dumps(di.get("secondary_ammo"))))
    ...
    def _write_atba(self, ship_id: str, letter: str, cs: dict, version_code: str = ""):
        ...
        # WG：副炮模块级配置（控制组 AUTO/SWITCHABLE + 手动模式修饰符）
        acfg = cs.get("_atba_config")
        if acfg:
            conn.execute("INSERT OR REPLACE INTO ship_module_atba_config (version_code, ship_id, config_group, control_groups_json, manual_mode_modifiers_json) VALUES (?,?,?,?,?)",
                         (version_code, ship_id, letter,
                          _json_dumps(acfg.get("control_groups")),
                          _json_dumps(acfg.get("manual_mode_modifiers"))))
    ...
    def _write_air_support(self, ship_id: str, letter: str, cs: dict, version_code: str = ""):
        """WG：反潜空袭模块（A_AirSupport 扁平结构）写入 ship_module_air_support"""
        items = cs.get("air_support", [])
        if not items:
            return
        self.conn.executemany(
            "INSERT OR REPLACE INTO ship_module_air_support "
            "(version_code, ship_id, config_group, module_key, plane_name, charges, reload_time, "
            " max_range, min_range, ammo_list_json, ammo_switch_coeff, fly_away_time, time_from_heaven, "
            " auto_use, available_buoyancy_states_json) "
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            [(version_code, ship_id, letter, i.get("plane_name") or "", i.get("plane_name"),
              i.get("charges"), i.get("reload_time"), i.get("max_range"), i.get("min_range"),
              _json_dumps(i.get("ammo_list")), i.get("ammo_switch_coeff"),
              i.get("fly_away_time"), i.get("time_from_heaven"), i.get("auto_use"),
              _json_dumps(i.get("available_buoyancy_states"))) for i in items])
        seen = set()
        for i in items:
            pn = i.get("plane_name") or ""
            if pn and (version_code, ship_id, pn) not in seen:
    ...
    def store_projectile(self, proj_id: str, raw_data: dict, version_code: str = ""):
        ...
        if species == "Torpedo":
            ...
            if stp or raw_data.get("searchRadius") is not None:
                ...
                conn.execute("INSERT OR REPLACE INTO projectile_torpedo_sub_guidance_ext (version_code, projectile_id, search_radius, search_angle, max_depth_level, max_vertical_speed, max_yaw, target_lost_degradation_time, drop_dist_aircarrier, drop_dist_battleship, drop_dist_cruiser, drop_dist_destroyer, drop_dist_submarine, drop_dist_default, params_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                             (version_code, proj_id,
                              _first(g.get("searchRadius")) or _first(raw_data.get("searchRadius")),
                              _first(g.get("searchAngle")) or _first(raw_data.get("searchAngle")),
                              _first(g.get("maxDepthLevel")) or _first(raw_data.get("maxDepthLevel")),
                              _first(g.get("maxVerticalSpeed")) or _first(raw_data.get("maxVerticalSpeed")),
                              _first(g.get("maxYaw")) or _first(raw_data.get("maxYaw")),
                              _first(g.get("targetLostDegradationTime")) or _first(raw_data.get("targetLostDegradationTime")),
                              _first(dr.get("AirCarrier")) or _first(raw_data.get("dropDistAircraftCarrier")),
                              _first(dr.get("Battleship")) or _first(raw_data.get("dropDistBattleship")),
                              _first(dr.get("Cruiser")) or _first(raw_data.get("dropDistCruiser")),
                              _first(dr.get("Destroyer")) or _first(raw_data.get("dropDistDestroyer")),
                              _first(dr.get("Submarine")) or _first(raw_data.get("dropDistSubmarine")),
                              _first(dr.get("default")) or _first(raw_data.get("dropDistDefault")),
                              _json_dumps(g)))
...
# ═════════════════════════════════════════════════════════════════════
# WargamingAnalysisService —— 分析编排器
# ═════════════════════════════════════════════════════════════════════

class WargamingAnalysisService:
    ...
    def _store_skills_and_minefields(self, db, other_dir, raw_conn, version_code=""):
        ...
        try:
            if other_dir.exists():
                store = WargamingAnalysisStore(db)
...
def _recognize_burst_wg(module_data: dict) -> dict | None:
    """WG 版弹夹炮识别"""
    for ck in ("SwitchableModeArtilleryModule",):
        conf = module_data.get(ck)
        if not conf:
            continue
        sec_ammo = conf.get("secondaryAmmoList")
        has_secondary_ammo = isinstance(sec_ammo, list) and len(sec_ammo) > 0
        # 修改型炮弹特征：存在副弹药列表 或 携带 modifiers（如 GMIdealRadius/GMPenetrationCoeffHE）
        has_modified = has_secondary_ammo or bool(conf.get("modifiers"))
        shots_count = conf.get("shotsCount")
        if has_modified and shots_count > 1:
            mode_name = "连发射击和修改型炮弹"
        elif has_modified:
            mode_name = "修改型炮弹"  # 情况 2：仅副弹药/修饰符非空（shots_count <= 1）
        else:
            mode_name = "连发射击"    # 情况 3：基础切换模式
        return {
            "name": "替代射击模式——" + mode_name,
            "shots_count": _scalar(conf.get("shotsCount")),
            # 连发间隔：SwitchableMode 用 burstReloadTime
            "shot_delay": _scalar(conf.get("burstReloadTime")),
            "full_reload_time": _scalar(conf.get("fullReloadTime")),
            "modifiers": conf.get("modifiers", {}),
            "secondary_ammo": conf.get("secondaryAmmoList"),
        }
    return None
