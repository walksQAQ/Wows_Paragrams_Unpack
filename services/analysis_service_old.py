...
class AnalysisStore:
    ...
    def store_ship(self, ship_id: str, raw_data: dict, result, version_code: str = "") -> None:
        ...
        # ── 基础属性（try/except 兜底，防止 FK 导致整舰跳过） ──
        try:
            self._write_basic_info(ship_id, raw_data, result, version_code=version_code)
        except Exception as e:
            bus.log_message.emit(f"  ⚠️ {ship_id} _write_basic_info 失败: {e}")
            # 如果 FK 失败，强制注册 entity 后重试一次
            try:
                ...
                self._write_basic_info(ship_id, raw_data, result, version_code=version_code)
                bus.log_message.emit(f"  ✅ {ship_id} _write_basic_info 重试成功")
            except Exception as e2:
                bus.log_message.emit(f"  ⚠️ {ship_id} _write_basic_info 重试仍失败: {e2}")
                # 重试仍失败，不 raise，让后续子操作继续执行（部分数据仍可写入）
    ...
    def _write_modules(self, ship_id: str, raw_data: dict,
        ...
        try:
            ...
            bus.log_message.emit(f"  ⚠️ {ship_id} _write_engine FK失败: {e}")
    ...
    def _write_engine(self, ship_id: str, raw_data: dict,
                       version_code: str = "") -> None:
        """从 EngineDefault 提取引擎数据写入 ship_module_engine"""
        conn = self.conn
        eng = raw_data.get("EngineDefault") or {}
        if not eng:
            return
        # 从船体数据获取最大航速作为 forward_max_speed
        hull_max_speed = None
        for mod_key, mod_data in raw_data.items():
            if not isinstance(mod_data, dict):
                continue
            ms = mod_data.get("maxSpeed")
            if ms:
                hull_max_speed = ms
                break
        forward_forsag = eng.get("forwardEngineForsag")
        backward_forsag = eng.get("backwardEngineForsag")
        forward_speed = hull_max_speed
        backward_speed = hull_max_speed * (1 + (eng.get("backwardSpeedOnFlood") or 0)) if hull_max_speed else None
        # 用 forwardEngineForsagMaxSpeed 作为 forsag 期间速度
        forsag_forward = eng.get("forwardEngineForsagMaxSpeed") or forward_forsag
        forsag_backward = eng.get("backwardEngineForsagMaxSpeed") or backward_forsag
        conn.execute("""INSERT OR REPLACE INTO ship_module_engine
            (version_code, ship_id, config_group, module_key,
             engine_type, engine_power,
             forward_max_speed, backward_max_speed,
             forward_forsage_power, backward_forsage_power)
            VALUES (?,?,?,?,?,?,?,?,?,?)""",
            (version_code, ship_id, "A", "EngineDefault",
             eng.get("engineType"),
             eng.get("histEnginePower") or eng.get("enginePower"),
             forward_speed, backward_speed,
             forsag_forward, forsag_backward))
    ...
    def _write_guns(self, conn, ship_id: str, letter: str,
        ...
        for (gun_name, barrels, reload_t, ir, mr, id_dist, ammo_tuple), group in groups.items():
            ...
            if is_artillery:
                conn.execute(f"""INSERT OR REPLACE INTO {table}
                    (version_code, ship_id, config_group, module_key, count, num_barrels,
                     ...
                     ideal_radius, min_radius, ideal_distance,
            ...
            # 写入 ship_weapon_projectiles（弹药关联，去重）
            if ammo_tuple:
                seen_ammo = set()
                for order, ammo_id in enumerate(ammo_tuple):
                    if ammo_id in seen_ammo:
                        continue
                    seen_ammo.add(ammo_id)
                    conn.execute("""INSERT OR REPLACE INTO ship_weapon_projectiles
                        (version_code, ship_id, module_id, slot_type, config_group, ammo_id, ammo_order)
                        VALUES (?,?,?,?,?,?,?)""",
                        (version_code, ship_id, gun_name, slot_type, letter, ammo_id, order))
    ...
    def _write_torpedoes(self, conn, ship_id, letter, items,
        ...
        for (name, barrels, reload_t), group in groups.items():
            ...
            conn.execute("""INSERT OR REPLACE INTO ship_module_relations
                (version_code, ship_id, module_id, slot_type, config_group, mount_count)
                VALUES (?,?,?,?,?,?)""",
                (version_code, ship_id, name, 'torpedo', letter, cnt))
            # 写入 ship_weapon_projectiles（去重）
            ref_item = group[0]
            seen_ammo = set()
            for order, ammo_id in enumerate(ref_item.get("ammo_list", []) or []):
                if ammo_id in seen_ammo:
                    continue
                seen_ammo.add(ammo_id)
                conn.execute("""INSERT OR REPLACE INTO ship_weapon_projectiles
                    (version_code, ship_id, module_id, slot_type, config_group, ammo_id, ammo_order)
                    VALUES (?,?,?,?,?,?,?)""",
                    (version_code, ship_id, name, 'torpedo', letter, ammo_id, order))
    ...
    def _write_aircraft(self, conn, ship_id, letter, items,
        ...
        if items:
            ...
            conn.executemany("""INSERT OR REPLACE INTO ship_module_aircraft
                (version_code, ship_id, config_group, module_key, module_variant, plane_type, plane_name, armament_name)
                VALUES (?,?,?,?,?,?,?,?)""", rows)
    ...
    def store_gun(self, gun_id: str, raw_data: dict, result, version_code: str = "") -> None:
        # 火炮作为独立实体仅注册到 entity_registry，详细属性随舰船模块写入 ship_module_artillery
        pass
        ...
        if species == "Artillery":
            conn.execute("""INSERT OR REPLACE INTO projectile_bullet_ext
                ...
                (version_code, proj_id,
                 raw_data.get("alphaDamage"), raw_data.get("bulletMass"),
                 raw_data.get("bulletSpeed"), raw_data.get("bulletDiametr"),
                 raw_data.get("bulletAirDrag"), raw_data.get("bulletKrupp"),
                 raw_data.get("alphaPiercingHE"), raw_data.get("explosionRadius"),
                 raw_data.get("burnProb"),
            ...
            conn.execute("""INSERT OR REPLACE INTO projectile_torpedo_ext
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (version_code, proj_id,
                 raw_data.get("bulletDiametr"), raw_data.get("alphaDamage"),
                 raw_data.get("damage"),
                 1 if raw_data.get("floodGeneration") else 0,
                 1 if raw_data.get("affectedByPTZ") else 0,
                 1 if raw_data.get("applyPTZCoeff") else 0,
                 raw_data.get("maxDist"),
                 raw_data.get("speed"), raw_data.get("visibilityFactor"),
                 raw_data.get("alertDist"), raw_data.get("armingTime"),
                 1 if raw_data.get("isDeepWater") else 0,
                 raw_data.get("deepWaterIgnoreClasses") or ""))

            # 潜艇声呐导向鱼雷扩增（数据位于 SubmarineTorpedoParams）
            def _unwrap(v):
                """从可能的单元素数组中解出标量值"""
            ...
            if stp or raw_data.get("searchRadius") is not None:
                ...
                drop = g.get("dropTargetAtDistance") or {}
                conn.execute("""INSERT OR REPLACE INTO projectile_torpedo_sub_guidance_ext
                    ...
                    (version_code, proj_id,
                     ...
                     _unwrap(drop.get("AirCarrier")) or raw_data.get("dropDistAircraftCarrier"),
                     _unwrap(drop.get("Battleship")) or raw_data.get("dropDistBattleship"),
                     _unwrap(drop.get("Cruiser")) or raw_data.get("dropDistCruiser"),
                     _unwrap(drop.get("Destroyer")) or raw_data.get("dropDistDestroyer"),
                     _unwrap(drop.get("Submarine")) or raw_data.get("dropDistSubmarine"),
                     _unwrap(drop.get("default")) or raw_data.get("dropDistDefault")))
            ...
            conn.execute("""INSERT OR REPLACE INTO projectile_rocket_ext
                ...
                (version_code, proj_id,
                 ...
                 raw_data.get("bulletMass"), raw_data.get("bulletSpeed"),
                 raw_data.get("bulletDiametr"), raw_data.get("bulletAirDrag"),
                 raw_data.get("alphaPiercingHE"), raw_data.get("burnProb"),
                 raw_data.get("explosionRadius"), raw_data.get("alphaPiercingCS"),
                 json.dumps(raw_data.get("attackSequenceDurations"), ensure_ascii=False) if isinstance(raw_data.get("attackSequenceDurations"), (list, dict)) else raw_data.get("attackSequenceDurations"),
                 raw_data.get("bulletKrupp"),
                 raw_data.get("bulletAlwaysRicochetAt"), raw_data.get("bulletRicochetAt"),
                 raw_data.get("bulletDetonator"), raw_data.get("bulletDetonatorThreshold"),
                 raw_data.get("bulletCapNormalizeMaxAngle")))
            ...
            conn.execute("""INSERT OR REPLACE INTO projectile_depth_charge_ext
                ...
                (version_code, proj_id,
                 ...
                 raw_data.get("timer"), raw_data.get("maxDepth"),
                 raw_data.get("depthSplashSize"), raw_data.get("depthSplashSizeToTorpedo")))

        elif species in ("SkipBomb",):
            # 跳弹炸弹复用 bomb_ext 表，标记 is_bomb=0 区分
            import json as _json
            skips = raw_data.get("skips") or raw_data.get("skipParams")
            conn.execute("""INSERT OR REPLACE INTO projectile_bomb_ext
                (version_code, projectile_id,
