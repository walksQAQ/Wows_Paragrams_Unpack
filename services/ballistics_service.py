...
class BallisticsCalculator:
    ...
    def generate_full_table(self, ballistics_data: dict, disp_params: dict, max_dist_km: float, step_km: float = 1.0, hoop_type: int = 0) -> dict:
        distances = []
        horizontal_disp = []
        vertical_disp = []
        disp_area = []
        ...
        max_dist_km = float(max_dist_km or 0.0)
        step_km = float(step_km or 1.0)
        if max_dist_km <= 0:
            return {
                "distances": distances,
        ...
        for r in [i * step_km for i in range(1, int(max_dist_km // step_km) + 1)]:
            if r > max_dist_km:
                continue
            distances.append(r)
            horiz = self.calc_horizontal_dispersion(r, disp_params)
            impact_deg = (ballistics_data.get("impact_angle_deg") or [0.0])[-1] if isinstance(ballistics_data.get("impact_angle_deg"), list) else 0.0
            vert = self.calc_vertical_dispersion(horiz, r, max_dist_km, disp_params, impact_deg, hoop_type)
            area = self.calc_dispersion_area(horiz, vert)
            horizontal_disp.append(horiz)
            vertical_disp.append(vert)
            disp_area.append(area)
            fly_time.append(ballistics_data.get("fly_time", [0.0])[-1] if isinstance(ballistics_data.get("fly_time"), list) else 0.0)
            penetration.append(ballistics_data.get("penetration", [0.0])[-1] if isinstance(ballistics_data.get("penetration"), list) else 0.0)
            impact_angle.append(impact_deg)
