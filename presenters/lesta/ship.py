...
class LestaShipPresenter(LestaBasePresenter):
    ...
    def build(self, ship_id: str, version_code: str = "", modifiers: dict | None = None,
        ...
        try:
            ...
            bus.log_message.emit(f"⚠️ [LestaShipPresenter] {ship_id} 构建失败: {e}\n{traceback.format_exc()}")
            return None
    ...
    def _append_consumables(self, conn, vc, ship_id, sections):
        ...
        for s in slots:
            ...
            if cfg:
                ...
                # 升级品/技能对「引擎增压消耗品」的修饰：
                #   可用次数 +1（speedBoostersAdditionalConsumables，加性）
                #   作用时间 −40%（speedBoostersWorkTimeCoeff 0.6，乘性）
                #   最高航速 +22%（boostCoeffForsage 0.22，**加性**加到自带 boostCoeff 上：0.08→0.30）
                _boost_uses = 0.0
                ...
                if ct == "crashCrew":
                    ...
                    # （boostCoeffForsage，如埃尔宾传奇插 +0.22 ⇒ +8% 变成 +30%）
    ...
    def _record_accel_ctx(self, letter, eng_row, hrow, cur_max_speed, tonnage,
        ...
        try:
            ...
            if boost:
                ...
                boost = dict(boost)
                # 升级品/技能对加力「最高航速加成」的**加性**修饰
                # （boostCoeffForsage，如埃尔宾传奇插 +0.22 ⇒ 增压极速从 +8% 变 +30%）
        ...
        for gi, g_row in enumerate(rows):
            ...
            if ext:
                ...
                if ext.get('special_mode_name'):
                    items, o = self._append_drum(ext, items, o)
                # 通用特殊属性
    ...
    def _append_drum(self, drum, items, o):
        ...
        if is_chargeable:
            items.append(self.make_item("连发射击-弹鼓模式", "", o, row_type="sub_header")); o += 1
            ...
            if cmode == 1:
                ...
                label = f"第 2 ~ {sc:.0f} 轮装填时间" if sc > 2 else "第 2 轮装填时间"
                items.append(self.make_item(label, f"{cmax}s", o)); o += 1
            elif cmode == 2:
                label = f"第 1 ~ {sc-1:.0f} 轮装填时间" if sc > 2 else "第 1 轮装填时间"
                items.append(self.make_item(label, f"{cmin}s", o)); o += 1
                items.append(self.make_item(f"第 {sc:.0f} 轮(末轮)装填时间", f"{cmax}s", o)); o += 1
        else:
            switch_prefix = "是" if is_switchable else "否"
            items.append(self.make_item(f"连发射击-替代射击模式", "", o, row_type="sub_header")); o += 1
            items.append(self.make_item("可切换", f"{switch_prefix}", o)); o += 1
        ...
        for grp_key, grp in group_map.items():
            ...
            for aid in ammo_ids:
                ...
                p = conn.execute(
                    ...
                    "te.deep_water_ignore_classes, te.affected_by_ptz "
                    "FROM projectile_basic_info pb "
