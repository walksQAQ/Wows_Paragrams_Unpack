...
| 层面 | 现状 | 说明 |
|------|------|------|
| 数据层解包（IDX/PKG/Kraken/bc7prep） | ✅ 已就绪 | `data_extractor.GameExtractor`（`list_files`/`extract`/`extract_single`）已按 glob 匹配 VFS 文件，docstring 示例即 `D:/World_of_Warships_RU/Korabli`，`content/**/*.geometry` 可直接提取，无需新写解包器 |
