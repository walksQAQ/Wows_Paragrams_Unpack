-- ═════════════════════════════════════════════════════════════════════
-- WARGAMING（WG）服专用架构 —— 与 Lesta 版独立，逐点调整
-- 检修：WG game_data 表结构差异只改本文件（不要动 lesta/database_new.sql）。
-- 加载：services/database_service.py 按 is_wg() 选本文件。
-- ═════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS ship_module_artillery_ext (
    ...
    switchablemode_shots_count INTEGER DEFAULT 0,
    switchablemode_shot_delay REAL DEFAULT 0,
    switchablemode_full_reload_time REAL DEFAULT 0,
    switchablemode_modifiers_json TEXT DEFAULT '{}',
    switchablemode_secondary_ammo_list TEXT DEFAULT '[]',
    PRIMARY KEY (version_code, ship_id, config_group, module_key),
...
-- 3b. 副炮模块配置表（WG：控制组 AUTO/SWITCHABLE + 手动模式修饰符）
CREATE TABLE IF NOT EXISTS ship_module_atba_config (
    version_code TEXT NOT NULL,
    ship_id TEXT NOT NULL,
    config_group TEXT NOT NULL,
    control_groups_json TEXT,                -- JSON: {"AUTO":[...],"SWITCHABLE":[...]}
    manual_mode_modifiers_json TEXT,         -- JSON: {"GSMMaxDist":1.173,"GSMShotDelay":0.6}
...
-- 8. 支援飞机组件属性表（WG：反潜空袭 A_AirSupport 扁平结构，含 ammo_list_json 等 WG 特有列）
CREATE TABLE IF NOT EXISTS ship_module_air_support (
    ...
    armament_name TEXT,
    support_type TEXT,
    ammo_list_json TEXT,                 -- WG：弹药 ID 列表（深水炸弹等）
    ammo_switch_coeff REAL,              -- WG：弹药切换系数
    fly_away_time REAL,                  -- WG：飞离时间(s)
    time_from_heaven REAL,               -- WG：天降时间(s)
    auto_use INTEGER,                    -- WG：自动使用 0/1
    available_buoyancy_states_json TEXT, -- WG：可用浮态列表
    PRIMARY KEY (version_code, ship_id, config_group, module_key),
    FOREIGN KEY (version_code, ship_id) REFERENCES ship_basic_info(version_code, ship_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS projectile_torpedo_sub_guidance_ext (
    ...
    params_json TEXT,                    -- WG：完整 SubmarineTorpedoParams（含三档位数组，如 dropTargetAtDistance 多档）
    PRIMARY KEY (version_code, projectile_id),
