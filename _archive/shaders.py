...
#
# 2026-08-03 从 Korabli64.exe <函数>（游戏内置反汇编器）逆向：
...
# - 指令长度表见 _LESTA_OP_LENGTH（<函数> 的 lVar21 更新提取，默认 1）
...
来自 exe 的 OP_0xXX 名字符串表（<地址> 附近），switch 未单独 case（走 default）。
"""

# exe 里 OP_0xXX 名字符串（<地址> 起，106 个）→ opcode 名映射
...
#: 每条指令的 u64 token 长度（从 <函数> 的 lVar21 更新提取，含 fall-through 继承；默认 1）
