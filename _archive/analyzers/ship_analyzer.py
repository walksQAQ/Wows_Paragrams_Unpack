...
class ShipSectionCollector:
    """
    舰船数据分节收集器 —— 替代 TextCollector。

    按模块/分区收集数据，每个 section 对应一种数据类型（基础属性、船体、消耗品等），
    最终生成 AnalysisResult 时保留多 section 结构，用于模块栏切换显示。
    """

    def __init__(self):
        self._sections: dict[str, list[DataItem]] = {}
        self._section_order: list[str] = []
        self._current_label: str | None = None
        self._current_items: list[DataItem] | None = None

    def begin_section(self, label: str) -> None:
        """开始一个新的分区（如"基础属性"、"船体模块数据"）"""
        if label not in self._sections:
            self._sections[label] = []
            self._section_order.append(label)
        self._current_label = label
        self._current_items = self._sections[label]

    def write(self, text: str) -> None:
        """添加一行文本到当前分区"""
        text = text.rstrip("\n")
        if self._current_items is not None:
            if text.strip():
                self._current_items.append(DataItem(name=text, value=""))
            else:
                self._current_items.append(DataItem(name="", value=""))

    def writeln(self, text: str = "") -> None:
        """添加一行文本到当前分区（含换行）"""
        self.write(text)

    def add_item(self, name: str, value: str = "", unit: str = "",
                 order: int = 0, raw_value=None) -> None:
        """向当前分区添加结构化条目（name/value/unit 分离）"""
        if self._current_items is not None:
            self._current_items.append(
                DataItem(name=name, value=str(value), unit=unit,
                         order=order, raw_value=raw_value)
            )
    ...
    def result(self, title: str, subtitle: str = "", extra: dict | None = None) -> AnalysisResult:
        """将各分区打包成 AnalysisResult"""
        merged_extra = {"module_order": self._section_order}
        if extra:
            merged_extra.update(extra)
        sub_sections = merged_extra.get("sub_sections", {})
        sections = [
            DataSection(label=label, items=self._sections[label])
            for label in self._section_order
            if self._sections[label] or label in sub_sections
        ]
...
# 向后兼容别名（TextCollector 已重命名为 ShipSectionCollector）
TextCollector = ShipSectionCollector
...
class ShipAnalyzer(BaseAnalyzer):
    ...
    def _build_hull_lines(self, hull_key: str, hull_data: dict, raw_species: str) -> list[str]:
        """渲染单个船体模块的文本行列表"""
        from models.name_mapping import Mapping as NameMapping
        lines = []
        ...
        for key in sorted(items, key=lambda x: items[x].get("order", 0)):
            ...
            name = item["name"]
            lines.append(f"    - {name}: {v}{unit_str}")
        # 潜艇特殊数据
        if raw_species == "Submarine":
            ...
            for entry in buoyancy_list:
                if isinstance(entry, dict) and entry.get('state') == 'DEEP_WATER_INVUL':
                    ...
                    break
            lines.append(f"    - 最大水下航速: {max_speed * invul_speed_multiplier:.2f} kts")
            ...
            for state_key in order:
                if state_key in b_map:
                    ...
                    lines.append(f"        ◈ [{cn_name}]: {d_range[0]:>5}m 至 {d_range[1]:>5}m")
            ...
            if has_battery:
                ...
                lines.append(f"    - 下潜能力:")
                lines.append(f"      - 基础电池容量: {bat_cap}")
                lines.append(f"      - 基础电力恢复速度: {bat_regen} /s")
            ...
            if has_hydrophone:
                ...
                lines.append(f"    - 水听器:")
                lines.append(f"      - 生效半径: {hp_radius} km")
                lines.append(f"      - 刷新周期: {hp_frep} s")
                if work_states:
                    lines.append(f"      - 水听器工作层级: {' / '.join(work_states)}")
                if detect_states:
                    lines.append(f"      - 可探测深度层级: {' / '.join(detect_states)}")
        lines.append("")
        return lines
    ...
    def analyze(self, raw_data: dict) -> AnalysisResult:
        ...
        将所有 display_area.insert() 替换为 ShipSectionCollector.write() / writeln()。
        """
        from models.name_mapping import Mapping as NameMapping
        t = ShipSectionCollector()
        ...
        for mod_key, module_data in raw_data.items():
            ...
            if current_cat == "AirArmament":
                for letter in target_letters:
                    combined_stats[letter]["AirArmament"] = module_data
                continue

            # 飞行控制（中队配置）
            if current_cat == "FlightControl":
                for letter in target_letters:
                    combined_stats[letter]["FlightControl"] = module_data
                continue
        ...
        # ── 模块数据（按字母分组渲染）────────────────────
        # 收集所有字母（船体+武器）
        all_letters: set[str] = set(combined_stats.keys())
        for hull_key in hulls_info.keys():
            letter = hull_key.split("_")[0] if "_" in hull_key else hull_key
            all_letters.add(letter)

        sub_section_info: dict[str, dict] = {}  # section_label -> {sub_labels, sub_contents}

        for letter in sorted(all_letters):
            section_label = f"{letter} 模块"
            t.begin_section(section_label)
            content_by_sub: dict[str, list[str]] = {}

            # ── 船体数据 ──
            hull_key_candidates = [k for k in hulls_info.keys()
                                   if (k.split("_")[0] if "_" in k else k) == letter]
            if hull_key_candidates:
                hull_lines: list[str] = []
                for hk in sorted(hull_key_candidates):
                    hull_lines.extend(self._build_hull_lines(hk, hulls_info[hk], raw_species))
                content_by_sub["船体"] = hull_lines

            # ── 武器数据 ──
            if letter in combined_stats:
                ...
                if stats.get("Artillery_System"):
                    ...
                    gun_lines.append(f"  主炮系统精度:")
                    gun_lines.append(f"    基础射程: {sys_info['max_dist'] / 1000:.1f} km" if sys_info['max_dist'] else "")
                    gun_lines.append(f"    Sigma: {sys_info['sigma']}")
                ...
                if artillery_items:
                    ...
                    for i in artillery_items:
                        ...
                        wp_counts[key] += 1
                    gun_lines.append(f"  主炮:")
                    for (name, barrels, reload, ir, mr, id_dist, rz, rd, rm, dl, ammo_tuple), count in wp_counts.items():
                        gun_lines.append(f"    - {name} x{count}")
                        gun_lines.append(f"      联装数: {barrels:.0f}")
                        gun_lines.append(f"      装填时间: {reload}s")
                        formula = self._dispersion_formula(ir, mr, id_dist)
                        if formula:
                            gun_lines.append(f"      横向散布公式: {formula}")
                        gun_lines.append(f"      纵向散步系数: {rz} ~ {rd} (R={dl * 100:.0f}%) ~ {rm}")
                        if ammo_tuple:
                            display_ammo = []
                            for a in ammo_tuple:
                                an = self.ammo_name_mapping.get(a.upper(), a)
                                display_ammo.append(f"{an} ({a})")
                            gun_lines.append(f"      可用弹药:")
                            for aitem in display_ammo:
                                gun_lines.append(f"        - {aitem}")
                # 弹夹/弹鼓信息
                if letter in drum_configs:
                    dm = drum_configs[letter]
                    gun_lines.append(f"  特殊射击模式: {dm['header']}")
                    for d in dm["details"]:
                        gun_lines.append(f"    - {d}")
                if gun_lines:
                    content_by_sub["主炮"] = gun_lines
                ...
                if atba_items:
                    ...
                    for i in atba_items:
                        key = (i["gun_name"], i.get("barrels", 0), i.get("reload", 0),
                               ...
                               tuple(sorted(i.get("ammo_list", []))))
                        wp_counts[key] += 1
                    atba_lines.append(f"  副炮:")
                    for (name, barrels, reload, ir, mr, id_dist, rz, rd, rm, dl, ammo_tuple), count in wp_counts.items():
                        atba_lines.append(f"    - {name} x{count}")
                        atba_lines.append(f"      联装数: {barrels:.0f}")
                        atba_lines.append(f"      装填时间: {reload}s")
                        formula = self._dispersion_formula(ir, mr, id_dist)
                        if formula:
                            atba_lines.append(f"      横向散布公式: {formula}")
                        atba_lines.append(f"      纵向散步系数: {rz} ~ {rd} (R={dl * 100:.0f}%) ~ {rm}")
                        if ammo_tuple:
                            display_ammo = []
                            for a in ammo_tuple:
                                an = self.ammo_name_mapping.get(a.upper(), a)
                                display_ammo.append(f"{an} ({a})")
                            atba_lines.append(f"      可用弹药:")
                            for aitem in display_ammo:
                                atba_lines.append(f"        - {aitem}")
                if stats.get("ATBA_System"):
                    sys_info = stats["ATBA_System"]
                    atba_lines.append(f"    副炮射程: {sys_info['max_dist'] / 1000:.1f} km" if sys_info.get('max_dist') else "")
                if atba_lines:
                    content_by_sub["副炮"] = atba_lines
                ...
                if torp_items:
                    ...
                    for i in torp_items:
                        key = (i["gun_name"], i.get("barrels", 0), i.get("reload", 0),
                               tuple(sorted(i.get("ammo_list", []))))
                        wp_counts[key] += 1
                    torp_lines.append(f"  鱼雷发射管:")
                    for (name, barrels, reload, ammo_tuple), count in wp_counts.items():
                        torp_lines.append(f"    - {name} x{count}")
                        torp_lines.append(f"      联装数: {barrels:.0f}")
                        torp_lines.append(f"      装填时间: {reload}s")
                        if ammo_tuple:
                            display_ammo = []
                            for a in ammo_tuple:
                                an = self.ammo_name_mapping.get(a.upper(), a)
                                display_ammo.append(f"{an} ({a})")
                            torp_lines.append(f"      可用弹药:")
                            for aitem in display_ammo:
                                torp_lines.append(f"        - {aitem}")
                if torp_lines:
                    content_by_sub["鱼雷"] = torp_lines
                    ...
                    for p in plane_items:
                        ...
                        for pl in planes:
                            if isinstance(pl, dict):
                                ...
                                plane_lines.append(f"  舰载机: {display}")
                                armament = pl.get("armamentName", "")
                                if armament:
                                    ammo_name = self.ammo_name_mapping.get(armament.upper(), armament)
                                    plane_lines.append(f"    弹药: {ammo_name} ({armament})")
                if plane_lines:
                    content_by_sub["舰载机"] = plane_lines

                # 机库/飞行甲板（AirArmament）
                aa_hangar_lines: list[str] = []
                aa_data = stats.get("AirArmament")
                if aa_data and isinstance(aa_data, dict):
                    deck_count = aa_data.get("deckPlaceCount", 0)
                    reserve = aa_data.get("planesReserveCapacity", 0)
                    launch_type = aa_data.get("launchpadType", "")
                    launch_prep = aa_data.get("launchPrepareTime", 0)
                    is_parallel = aa_data.get("isParallelLaunch", False)
                    joint_count = aa_data.get("jointLaunchPlaneCount", 1)
                    joint_delay = aa_data.get("jointLaunchDelay", 0)
                    aa_hangar_lines.append(f"  机库/飞行甲板:")
                    aa_hangar_lines.append(f"    甲板车位: {deck_count}")
                    aa_hangar_lines.append(f"    预备载机量: {reserve}")
                    aa_hangar_lines.append(f"    弹射器类型: {launch_type}")
                    aa_hangar_lines.append(f"    起飞准备时间: {launch_prep}s")
                    aa_hangar_lines.append(f"    联动弹射: {'是' if is_parallel else '否'}")
                    aa_hangar_lines.append(f"    联动弹射架次: {joint_count}")
                    aa_hangar_lines.append(f"    联动弹射间隔: {joint_delay}s")
                    # 机库血量
                    hangar_item = aa_data.get("hangar_1", {})
                    if hangar_item and isinstance(hangar_item, dict):
                        hp = hangar_item.get("maxHP", 0)
                        regen = hangar_item.get("regeneratedHPPart", 0)
                        aa_hangar_lines.append(f"    机库血量: {hp:.0f}")
                        aa_hangar_lines.append(f"    机库回复率: {regen:.0%}")
                fc_data = stats.get("FlightControl")
                if fc_data and isinstance(fc_data, dict):
                    air_support = fc_data.get("airSupportSquadrons", [])
                    if air_support:
                        aa_hangar_lines.append(f"    空中支援中队: {len(air_support)} 组")
                if aa_hangar_lines:
                    content_by_sub["机库"] = aa_hangar_lines

                # 空袭
                as_lines: list[str] = []
                as_items = stats.get("AirSupport", [])
                if as_items:
                    as_lines.append(f"  空袭/支援:")
                    for item in as_items:
                        plane_display = self.get_localized_plane_name(item.get('plane_id', ''))
                        as_lines.append(f"    - 飞机: {plane_display}")
                        if item.get('charges'):
                            as_lines.append(f"      次数: {item['charges']}")
                        if item.get('reload'):
                            as_lines.append(f"      装填: {item['reload']}s")
                        armament = item.get("armament", "")
                        if armament:
                            ammo_name = self.ammo_name_mapping.get(armament.upper(), armament)
                            as_lines.append(f"      弹药: {ammo_name} ({armament})")
                if as_lines:
                    content_by_sub["空袭"] = as_lines

            # ── 子分类数据仅存入 extra，不写入 items 列表 ──
            sub_labels = list(content_by_sub.keys())

            sub_section_info[section_label] = {
                "sub_labels": sub_labels,
                "sub_contents": {k: list(v) for k, v in content_by_sub.items()},
            }

        return t.result(
            title=real_name,
            subtitle=f"ID: {ship_index} | {NameMapping.SHIP_CLASS_MAP.get(raw_species, raw_species)}",
